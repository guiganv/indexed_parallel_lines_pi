...
extern indexed_parallel_lines_pi    *g_indexed_parallel_lines_pi;
...
tpControlDialogImpl::tpControlDialogImpl( wxWindow* parent ) : tpControlDialogDef( parent )
{
    ...
    if(!g_indexed_parallel_lines_pi->m_bODCreateBoundary) m_buttonCreateBoundaryODAPI->Disable();
    if(!g_indexed_parallel_lines_pi->m_bODCreateBoundaryPoint) m_buttonCreateBoundaryPointODAPI->Disable();
    if(!g_indexed_parallel_lines_pi->m_bODCreateTextPoint) m_buttonCreateTextPointODAPI->Disable();
    if(g_indexed_parallel_lines_pi->m_fnOutputJSON == wxEmptyString) m_checkBoxSaveJSON->Disable();
...
void tpControlDialogImpl::OnButtonClickCreateBoundaryODAPI( wxCommandEvent& event )
{
    ...
    g_indexed_parallel_lines_pi->CreateBoundary(pCB);
    g_indexed_parallel_lines_pi->ToggleToolbarIcon();
    Show(false);
    delete pCBP1;
    delete pCBP2;
    delete pCBP3;
...
void tpControlDialogImpl::OnButtonClickDeleteBoundaryODAPI(wxCommandEvent& event)
{
    ...
    g_indexed_parallel_lines_pi->DeleteBoundary(pDB);
}
...
void tpControlDialogImpl::OnButtonClickDeleteBoundaryJSON( wxCommandEvent& event )
{
    ...
    jMsg[wxT("Source")] = wxT("INDEXED_PARALLEL_LINES_PI");
...
void tpControlDialogImpl::OnButtonClickCreateBoundaryPointODAPI( wxCommandEvent& event )
{
    ...
    bool ret = g_indexed_parallel_lines_pi->CreateBoundaryPoint(pCBP);
    g_indexed_parallel_lines_pi->ToggleToolbarIcon();
    Show(false);
}
...
void tpControlDialogImpl::OnButtonClickDeleteBoundaryPointODAPI(wxCommandEvent& event)
{
    ...
    g_indexed_parallel_lines_pi->DeleteBoundaryPoint(pDBP);
}
...
void tpControlDialogImpl::OnButtonClickCreateTextPointODAPI( wxCommandEvent& event )
{
    ...
    g_indexed_parallel_lines_pi->CreateTextPoint(pCTP);
    g_indexed_parallel_lines_pi->ToggleToolbarIcon();
    Show(false);
}
...
void tpControlDialogImpl::OnButtonClickDeleteTextPointODAPI(wxCommandEvent& event)
{
    ...
    g_indexed_parallel_lines_pi->DeleteTextPoint(pDTP);
}
...
void tpControlDialogImpl::OnButtonClickPointIconODAPI(wxCommandEvent& event)
{
    if(m_radioBoxPointIcon->GetSelection() == 0) {
        ...
        g_indexed_parallel_lines_pi->AddPointIcon(pAPI);
        g_indexed_parallel_lines_pi->ToggleToolbarIcon();
        Show(false);
    } else {
        DeletePointIcon_t *pDPI = new DeletePointIcon_t;
        pDPI->PointIconName = _T("blue_anchor");
        g_indexed_parallel_lines_pi->DeletePointIcon(pDPI);
        g_indexed_parallel_lines_pi->ToggleToolbarIcon();
        Show(false);
    }
}
...
void tpControlDialogImpl::OnButtonClickGetGUIDSODAPI(wxCommandEvent& event)
{
  ...
  g_indexed_parallel_lines_pi->GetGUIDList(pGL);
...
void tpControlDialogImpl::tpControlOnClickImportJSON( wxCommandEvent& event )
{
    ...
    if(m_checkBoxSaveJSON->IsChecked()) {
        ...
        g_indexed_parallel_lines_pi->m_fnOutputJSON = m_filePickerOutputJSON->GetPath();
    } else {
        g_indexed_parallel_lines_pi->m_fnOutputJSON = wxEmptyString;
    }

    if(g_indexed_parallel_lines_pi->m_fnInputJSON.GetFullPath() == wxEmptyString) {
        OCPNMessageBox_PlugIn( NULL, _("No file specified for input"), _("File not found"), wxICON_EXCLAMATION | wxCANCEL );
        return;
    }
    g_indexed_parallel_lines_pi->m_fnInputJSON = m_filePickerInputJSON->GetPath();

    g_indexed_parallel_lines_pi->ImportJSONFile();
    g_indexed_parallel_lines_pi->ToggleToolbarIcon();
    Show(false);
}
...
{
    if(g_indexed_parallel_lines_pi->m_bODCreateBoundary) m_buttonCreateBoundaryODAPI->Enable();
    else m_buttonCreateBoundaryODAPI->Disable();
    if(g_indexed_parallel_lines_pi->m_bODCreateBoundaryPoint) m_buttonCreateBoundaryPointODAPI->Enable();
    else m_buttonCreateBoundaryPointODAPI->Disable();
    if(g_indexed_parallel_lines_pi->m_bODCreateTextPoint) m_buttonCreateTextPointODAPI->Enable();
...
{
    if(m_checkBoxSaveJSON->IsChecked()) g_indexed_parallel_lines_pi->m_bSaveIncommingJSONMessages = true;
    else g_indexed_parallel_lines_pi->m_bSaveIncommingJSONMessages = false;
}

void tpControlDialogImpl::OnCheckBoxCloseSaveFileAfterEachWrite( wxCommandEvent& event )
{
    g_indexed_parallel_lines_pi->UpdateCloseAfterSave(m_checkBoxCloseAferWrite->IsChecked());
    if(m_checkBoxCloseAferWrite->IsChecked()) g_indexed_parallel_lines_pi->m_bCloseSaveFileAfterEachWrite = true;
    else g_indexed_parallel_lines_pi->m_bCloseSaveFileAfterEachWrite = false;
}

void tpControlDialogImpl::OnCheckBoxAppendToFile(wxCommandEvent& event)
{
    g_indexed_parallel_lines_pi->UpdateAppendToFile(m_checkBoxAppendToFile->IsChecked());
    if(m_checkBoxAppendToFile->IsChecked()) g_indexed_parallel_lines_pi->m_bAppendToSaveFile = true;
    else g_indexed_parallel_lines_pi->m_bAppendToSaveFile = false;
}

void tpControlDialogImpl::OnCheckBoxDeleteFromConfig(wxCommandEvent& event)
{
    g_indexed_parallel_lines_pi->m_bRecreateConfig = m_checkBoxRecreateConfig->IsChecked();
}
