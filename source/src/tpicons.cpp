...
//    fn.AppendDir(wxT("indexed_parallel_lines_pi"));
//#else
//    fn.SetPath(*GetpSharedDataLocation());
    //const char *sPluginName = "indexed_parallel_lines_pi";
    //fn.SetPath(GetPluginDataDir(sPluginName));
    //const char *sPluginName = "indexed_parallel_lines_pi";
    fn.SetPath(GetPluginDataDir("indexed_parallel_lines_pi"));
    //    fn.AppendDir( wxT("plugins") );
//    fn.AppendDir(wxT("indexed_parallel_lines_pi"));
    fn.AppendDir(wxT("data"));
    g_SData_Locn = new wxString(fn.GetFullPath().c_str());
//#endif
    wxString s = _("indexed_parallel_lines_pi data location");
    wxLogMessage( wxT("%s: %s"), s.c_str(), fn.GetFullPath().c_str());

    m_failedBitmapLoad = false;

#ifdef PLUGIN_USE_SVG
    fn.SetFullName(wxT("indexed_parallel_lines.svg"));
    m_s_indexed_parallel_lines_pi = fn.GetFullPath();
    m_bm_indexed_parallel_lines_pi = LoadSVG( fn.GetFullPath() );
    fn.SetFullName(wxT("indexed_parallel_linesgrey.svg"));
    m_s_indexed_parallel_lines_grey_pi = fn.GetFullPath();
    m_bm_indexed_parallel_lines_grey_pi = LoadSVG( fn.GetFullPath() );
    fn.SetFullName(wxT("indexed_parallel_lines-toggled.svg"));
    m_s_indexed_parallel_lines_toggled_pi = fn.GetFullPath();
    m_bm_indexed_parallel_lines_toggled_pi = LoadSVG( fn.GetFullPath() );
#else
    fn.SetFullName(wxT("indexed_parallel_lines.png"));
    m_p_bm_indexed_parallel_lines_pi = new wxBitmap( fn.GetFullPath(), wxBITMAP_TYPE_PNG );
    if(!m_p_bm_indexed_parallel_lines_pi->IsOk())  m_failedBitmapLoad = true;
#endif

    if(m_failedBitmapLoad) {
        int ret = OCPNMessageBox_PlugIn( NULL, _("Failed to load all indexed_parallel_lines_pi icons, check OCPN log for details"), _("OpenCPN Alert"), wxOK );
    } else {
        CreateSchemeIcons();
        ScaleIcons();
    }
...
wxBitmap tpicons::LoadSVG( const wxString filename, unsigned int width, unsigned int height )
{
    ...
    wxString s = _("indexed_parallel_lines_pi LoadSVG");
...
bool tpicons::ScaleIcons()
{
    ...
    //m_bm_indexed_parallel_lines_pi = ScaleIcon( m_p_svgd_indexed_parallel_lines_pi, m_p_img_indexed_parallel_lines_pi, m_dScaleFactor );
    //m_bm_indexed_parallel_lines_grey_pi = ScaleIcon( m_p_svgd_indexed_parallel_lines_grey_pi, m_p_img_indexed_parallel_lines_grey_pi, m_dScaleFactor );
...
void tpicons::ChangeScheme(void)
{
    switch(m_ColourScheme) {
        ...
        case PI_GLOBAL_COLOR_SCHEME_DAY:
            m_bm_indexed_parallel_lines_grey_pi = m_bm_day_indexed_parallel_lines_grey_pi;
            break;
        case PI_GLOBAL_COLOR_SCHEME_DUSK:
            m_bm_indexed_parallel_lines_grey_pi = m_bm_dusk_indexed_parallel_lines_grey_pi;
            break;
        case PI_GLOBAL_COLOR_SCHEME_NIGHT:
            m_bm_indexed_parallel_lines_grey_pi = m_bm_night_indexed_parallel_lines_grey_pi;
            break;
        default:
            break;
    }
...
{
    m_bm_day_indexed_parallel_lines_grey_pi = m_bm_indexed_parallel_lines_grey_pi;
    m_bm_day_indexed_parallel_lines_toggled_pi = m_bm_indexed_parallel_lines_toggled_pi;
    m_bm_day_indexed_parallel_lines_pi = m_bm_indexed_parallel_lines_pi;
    m_bm_dusk_indexed_parallel_lines_grey_pi = BuildDimmedToolBitmap(m_bm_indexed_parallel_lines_grey_pi, 128);
    m_bm_dusk_indexed_parallel_lines_pi = BuildDimmedToolBitmap(m_bm_indexed_parallel_lines_pi, 128);
    m_bm_dusk_indexed_parallel_lines_toggled_pi = BuildDimmedToolBitmap(m_bm_indexed_parallel_lines_toggled_pi, 128);
    m_bm_night_indexed_parallel_lines_grey_pi = BuildDimmedToolBitmap(m_bm_indexed_parallel_lines_grey_pi, 32);
    m_bm_night_indexed_parallel_lines_pi = BuildDimmedToolBitmap(m_bm_indexed_parallel_lines_pi, 32);
    m_bm_night_indexed_parallel_lines_toggled_pi = BuildDimmedToolBitmap(m_bm_indexed_parallel_lines_toggled_pi, 32);
