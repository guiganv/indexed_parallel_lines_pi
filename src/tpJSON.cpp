...
void tpJSON::ProcessMessage(wxString &message_id, wxString &message_body)
{
    ...
    if(g_indexed_parallel_lines_pi->m_bSaveIncommingJSONMessages) {
        if(!m_ffOutputFile) {
            wxString l_mode;
            if(g_indexed_parallel_lines_pi->m_bAppendToSaveFile) l_mode = "a";
            else l_mode = "w";
            m_ffOutputFile  = new wxFFile(g_indexed_parallel_lines_pi->m_fnOutputJSON.GetFullPath(), l_mode);
            if(!m_ffOutputFile->IsOpened()) {
                g_indexed_parallel_lines_pi->m_bSaveIncommingJSONMessages = false;
                OCPNMessageBox_PlugIn(NULL, g_indexed_parallel_lines_pi->m_fnOutputJSON.GetPath(), _("File not found"), wxICON_EXCLAMATION | wxCANCEL);
            }
        }
        ...
        if(g_indexed_parallel_lines_pi->m_bCloseSaveFileAfterEachWrite) {
            m_ffOutputFile->Flush();
            m_ffOutputFile->Close();
            delete m_ffOutputFile;
            m_ffOutputFile = NULL;
    ...
    if(message_id != _T("INDEXED_PARALLEL_LINES_PI")) {
        if(message_id == _T("OCPN_DRAW_PI_READY_FOR_REQUESTS")) {
            if(message_body == _T("TRUE")) {
                if(g_indexed_parallel_lines_pi->m_bReadyForRequests)
                    g_indexed_parallel_lines_pi->GetODAPI();
            }
        }
    } else if(message_id == wxS("INDEXED_PARALLEL_LINES_PI")) {
        ...
        if ( numErrors > 0 )  {
            ...
            for(size_t i = 0; i < errors.GetCount(); i++)
            {
                if(i == 0) {
                    sLogMessage.clear();
                    sLogMessage.Append(wxT("indexed_parallel_lines_pi: Error parsing JSON message - "));
