...
#ifndef __WXMSW__
    if(g_iLocaleDepth == 0) {
        g_tplocale = new wxString(wxSetlocale(LC_NUMERIC, NULL));
...
void ResetGlobalLocale( void )
{
    ...
    if(g_iLocaleDepth < 0)
        g_iLocaleDepth = 0;
    if(g_iLocaleDepth == 0 && g_tplocale) {
#if wxCHECK_VERSION(3,0,0)
        wxSetlocale(LC_NUMERIC, g_tplocale->ToAscii());
#else
        setlocale(LC_NUMERIC, g_tplocale->ToAscii());
#endif
        delete g_tplocale;
        g_tplocale = NULL;
    }
